...
This guide walks through deploying BioCLIP Image Search Lite to [Hugging Face Spaces](https://huggingface.co/spaces). The app is currently live at **[netzhang/bioclip-image-search-lite](https://huggingface.co/spaces/netzhang/bioclip-image-search-lite)**.
...
The workflow at `.github/workflows/sync-to-hf-space.yml` pushes code to the HF Space on every commit to `main`:
  ...
  sync:
    ...
    steps:
      ...
      - name: Push to HF Space
        ...
        run: git push https://netzhang:$HF_TOKEN@huggingface.co/spaces/netzhang/bioclip-image-search-lite main
```

This is a simple `git push` — HF Spaces treats its git repo as the source of truth. When code arrives, HF rebuilds and restarts the Space automatically.
...
| What | Location |
|------|----------|
| Live app | `https://netzhang-bioclip-image-search-lite.hf.space` |
| Space settings | `https://huggingface.co/spaces/netzhang/bioclip-image-search-lite/settings` |
| Build & runtime logs | `https://huggingface.co/spaces/netzhang/bioclip-image-search-lite/logs` (visible from the three-dot menu or the "Logs" tab at the bottom of the Space page) |
| Data repo (FAISS + DuckDB) | `https://huggingface.co/netzhang/bioclip-image-search-lite` |
